...
Create a folder named graphapp, or a name of your choosing, for the project.
...
dotnet new console
Run the following commands to add the Azure.Identity, Microsoft.Graph, and the dotenv.net packages to the project.
...
Locate the // ADD CODE TO DEFINE SCOPE AND CONFIGURE AUTHENTICATION comment and add the following code directly after the comment. Be sure to review the comments in the code.
...
Locate the // ADD CODE TO CREATE GRAPH CLIENT AND RETRIEVE USER PROFILE comment and add the following code directly after the comment. Be sure to review the comments in the code.
...
async Task GetUserProfile(GraphServiceClient graphClient)
{
    try
    {
        ...
        // Note: explicitly selecting fields is required in some tenant configurations
...
In the Azure portal, navigate to the app registration you created.
